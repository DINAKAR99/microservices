...
public class MovieCatalogeController {
    ...
    public MovieCatalogeController(RestTemplate rt, RestClient restClient, RestClient.Builder restClientBuilder) {
        this.rt = rt;
    ...
    public List<CatalogItem> getCatalog(@PathVariable("userId") String userId) {
        ...
        UserRatings ratings1 = restClientBuilder.build().get().uri("http://ratings-data-service/ratingsdata/users/"
                +
                userId)
                .retrieve()
                .body(UserRatings.class);
        ...
        return ratings1.getUser_ratings().stream().map(rating -> {
            ...
            // -----------------------------
            Movie movie = restClientBuilder.build().get().uri("http://movie-info-service/movies/" +
                    rating.getMovieId()).retrieve()
                    .body(Movie.class);
            // -----------------------------

            // create each catlog item for each rating
            return new CatalogItem(movie.getName(), "suit build", rating.getRating());
